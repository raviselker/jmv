...
ttestOneSClass <- R6::R6Class(
    ...
    private = list(
        .run = function() {
            ...
            ## Listwise NA cleanup
            if (naHandling == "listwise") dataset <- naOmit(dataset)
            ...
            for (i in seq_along(variables)) {
                ...
                if (self$options$get("students")) {
                    ...
                    if (!isError(res)) {
                        ...
                        if (message == "not enough 'x' observations")
                            message <- .('Variable does not contain enough observations') else if (
                ...
                if (self$options$wilcoxon || self$options$mann) {
                    if (is.factor(column))
                        ...
                        res <- createError(.('Variable does not contain enough observations')) else
                        res <- try(
                ...
                if (n < 3) {
                    ...
                    if (isError(res)) res <- list(statistic = NaN, p.value = '')
                }
                ...
                if (self$options$get('plots')) {
                    ...
                    plotData <- rbind(
                        plotData,
                        list(stat = med, cie = NA, type = 'median', var = name),
                        stringsAsFactors = FALSE
                    )
        ...
        .init = function() {
            ...
            if (hypothesis == 'dt')
                table$setNote(
                    "hyp",
                    jmvcore::format("H\u2090 \u03BC \u2260 {}", testValue)
                ) else if (hypothesis == 'gt')
                table$setNote("hyp", jmvcore::format("H\u2090 \u03BC > {}", testValue)) else if (
                hypothesis == 'lt'
            )
        ...
        .desc = function(image, ggtheme, theme, ...) {
            ...
            plot <- ggplot(data = image$state, aes(x = var, y = stat, shape = type)) +
                ...
                geom_errorbar(
                    aes(x = var, ymin = stat - cie, ymax = stat + cie, shape = type, width = .1),
                    size = .8,
                    colour = theme$color[1],
                    position = pd
                ) +
                geom_point(
                    aes(x = var, y = stat, colour = type, shape = type),
                    fill = theme$fill[1],
                    size = 3,
                    colour = theme$color[1],
                    position = pd
