anovaNPClass <- R6::R6Class(
    ...
    private = list(
        .run = function() {
            ...
            if (length(deps) == 0 || is.null(group)) return()
        ...
        .genPairs = function(groupColumn) {
            ...
            if (length(groupLevels) > 0) {
                ...
                for (i in seq_len(dim(pairsMatrix)[2])) pairsList[[i]] <- pairsMatrix[, i]
            }
...
pSDCFlig <- function(x, g = NA, method = NA, n.mc = 10000, n.g) {
    ...
    ##From kruskal.test()##
    if (is.list(x)) {
        if (length(x) < 2L) stop("'x' must be a list with at least 2 elements")
        DNAME <- deparse(substitute(x))
        x <- lapply(x, function(u) u <- u[complete.cases(u)])
        k <- length(x)
        l <- sapply(x, "length")
        if (any(l == 0)) stop("all groups must contain data")
        g <- factor(rep(1:k, l))
        x <- unlist(x)
    } else {
    ...
    W.star.calc <- function(x, i, j) {
        group.sizes <- l[c(i, j)]
        W.stat <- sum(rank(c(x[g == levels(g)[i]], x[g == levels(g)[j]]))[
            (group.sizes[1] + 1):sum(group.sizes)
        ])
        W.mean <- group.sizes[2] * (sum(group.sizes) + 1) / 2
        tie.vec <- as.numeric(table(c(x[g == levels(g)[i]], x[g == levels(g)[j]])))
        W.var <- prod(group.sizes) /
            24 *
            (sum(group.sizes) +
                1 -
                sum(
                    (tie.vec - 1) *
                        tie.vec *
                        (tie.vec + 1) /
                        (sum(group.sizes) * (sum(group.sizes) - 1))
                ))
