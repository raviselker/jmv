#' @importFrom jmvcore .
logRegBinClass <- if (requireNamespace('jmvcore'))
    R6::R6Class(
        "logRegBinClass",
        ...
        active = list(
            ...
            formulas = function() {
                if (is.null(private$.formulas)) private$.formulas <- private$.getFormulas()

                return(private$.formulas)
            },
            models = function() {
                if (is.null(private$.models)) private$.models <- private$.computeModels()

                return(private$.models)
            },
            nModels = function() {
                if (is.null(private$.nModels)) private$.nModels <- length(self$options$blocks)

                return(private$.nModels)
            },
            residuals = function() {
                if (is.null(private$.residuals)) private$.residuals <- private$.computeResiduals()

                return(private$.residuals)
            },
            fitted = function() {
                if (is.null(private$.fitted)) private$.fitted <- private$.computeFitted()

                return(private$.fitted)
            },
            predicted = function() {
                if (is.null(private$.predicted)) private$.predicted <- private$.computePredicted()

                return(private$.predicted)
            },
            cooks = function() {
                if (is.null(private$.cooks)) private$.cooks <- private$.computeCooks()
            ...
            deviance = function() {
                if (is.null(private$.deviance)) private$.deviance <- private$.computeDeviance()

                return(private$.deviance)
            },
            AIC = function() {
                if (is.null(private$.AIC)) private$.AIC <- private$.computeAIC()

                return(private$.AIC)
            },
            BIC = function() {
                if (is.null(private$.BIC)) private$.BIC <- private$.computeBIC()

                return(private$.BIC)
            },
            pseudoR2 = function() {
                if (is.null(private$.pseudoR2)) private$.pseudoR2 <- private$.computePseudoR2()

                return(private$.pseudoR2)
            },
            modelTest = function() {
                if (is.null(private$.modelTest)) private$.modelTest <- private$.computeModelTest()

                return(private$.modelTest)
            },
            VIF = function() {
                if (is.null(private$.VIF)) private$.VIF <- private$.computeVIF()

                return(private$.VIF)
            },
            CICoefEst = function() {
                if (is.null(private$.CICoefEst)) private$.CICoefEst <- private$.computeCICoefEst()
            ...
            AUC = function() {
                if (is.null(private$.AUC)) private$.AUC <- private$.computeAUC()

                return(private$.AUC)
            },
            emMeans = function() {
                if (is.null(private$.emMeans)) private$.emMeans <- private$.computeEmMeans()
        ...
        private = list(
            ...
            #### Init + run functions ----
            .init = function() {
                if (!is.null(self$options$dep)) private$.errorCheck()
            ...
            .computeModels = function(modelNo = NULL) {
                ...
                if (is.numeric(modelNo)) formulas <- formulas[modelNo]
            ...
            .computeWeights = function() {
                ...
                if (is.null(global_weights)) return()
            ...
            .computeResiduals = function() {
                res <- list()
                for (i in seq_along(self$models)) res[[i]] <- self$models[[i]]$residuals

                return(res)
            },
            .computeFitted = function() {
                fitted <- list()
                for (i in seq_along(self$models)) fitted[[i]] <- self$models[[i]]$fitted.values
            ...
            .computeDeviance = function() {
                dev <- list()
                for (i in seq_along(self$models)) dev[[i]] <- self$models[[i]]$deviance

                return(dev)
            },
            .computeAIC = function() {
                AIC <- list()
                for (i in seq_along(self$models)) AIC[[i]] <- stats::AIC(self$models[[i]])

                return(AIC)
            },
            .computeBIC = function() {
                BIC <- list()
                for (i in seq_along(self$models)) BIC[[i]] <- stats::BIC(self$models[[i]])
                    ...
                    for (j in seq_along(terms)) {
                        if (any(terms[[j]] %in% factors)) {
                            ...
                            for (k in seq_along(contrastNames)) {
                                ...
                                table$addRow(
                                    rowKey = rowKey,
                                    values = list(term = contrastNames[[k]])
                                )
            ...
            .initClassTable = function() {
                ...
                for (i in seq_along(self$options$blocks)) {
                    ...
                    table$setRow(
                        rowNo = 1,
                        values = list('name[0]' = levels[1], 'name[1]' = levels[2])
                    )
            ...
            .initCollinearityTable = function() {
                ...
                for (i in seq_along(termsAll)) {
                    ...
                    for (i in seq_along(terms))
                        table$addRow(
                            rowKey = i,
                            values = list(term = jmvcore::stringifyTerm(terms[i]))
                        )
                }
            },
            .initOutputs = function() {
                ...
                if (self$nModels == 1) {
                    self$results$predictOV$set(
                        1,
                        title(predictTitle, dep),
                        description(predictDescPrefix),
                        'continuous'
                    )
                    self$results$residsOV$set(
                        1,
                        residsTitle,
                        description(residsTitle),
                        'continuous'
                    )
                    ...
                    descriptions <- vapply(
                        keys,
                        function(key) description(predictDescPrefix, key),
                        ''
                    )
            ...
            .populateEmmTables = function() {
                if (!self$options$emmTables) return()
                        ...
                        if (!is.null(emm) && all(emm %in% terms)) {
                            ...
                            for (k in seq_along(emm)) {
                                if (emm[k] %in% covs)
                                    covValues[[emm[
                                        k
                                    ]]] <- sort(unique(emmTable[, jmvcore::toB64(emm[k])]))
                            }
            ...
            .populatePredMeasuresTable = function() {
                ...
                for (i in seq_along(groups)) {
                    ...
                    if (self$options$auc) row[['auc']] <- self$AUC[[i]]

                    table$setRow(rowNo = 1, values = row)
                }
            },
            .populateCollinearityTable = function() {
                if (!self$options$collin) return()
                ...
                for (i in seq_along(termsAll)) {
                    ...
                    if (length(self$VIF) == 0) VIF <- NULL else VIF <- self$VIF[[i]]
                    ...
                    for (i in seq_along(terms)) {
                        ...
                        if (length(terms) <= 1) {
                            ...
                            # check which rows have the same length + same terms
                            index <- which(
                                length(terms[[i]]) == sapply(rowTerms, length) &
            ...
            .prepareEmmPlots = function() {
                if (!self$options$emmPlots) return()
                    ...
                    for (i in seq_along(termsAll)) {
                        ...
                        for (term in terms) {
                            if (any(term %in% factors))
                                rowNames <- c(rowNames, private$.contrastModel(term)) else
            ...
            .cleanData = function(naOmit = TRUE, naSkip = NULL) {
                ...
                for (cov in covs) data[[jmvcore::toB64(cov)]] <- jmvcore::toNumeric(dataRaw[[cov]])
            ...
            .contrastModel = function(terms) {
                ...
                nLevels <- list()
                for (factor in factors) nLevels[[factor]] <- length(levels(self$data[[factor]]))
            ...
            .contrastsCoefTable = function(terms) {
                ...
                levels <- list()
                for (factor in factors) levels[[factor]] <- levels(self$data[[factor]])
