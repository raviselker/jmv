anovaRMNPClass <- R6::R6Class(
    ...
    private = list(
        ...
        .run = function() {
            ...
            if (length(measureNames) < 2) return()

            data <- select(self$data, measureNames)
            data <- jmvcore::naOmit(data)
            mat <- matrix(
                nrow = nrow(data),
                ncol = length(measureNames),
                dimnames = list(NULL, measureNames)
            )
            ...
            for (m in measureNames) {
                ...
                desc$setRow(
                    rowKey = m,
                    list(mean = descData[index, 'mean'], median = descData[index, 'median'])
                )
            }
        },
        .init = function() {
            ...
            for (m in measureNames) desc$addRow(rowKey = m, list(level = m))
        },
        .preparePlot = function(data) {
            for (i in seq_along(data)) data[[i]] <- jmvcore::toNumeric(data[[i]])

            dataNew <- reshape2:::melt.data.frame(
                data,
                measure.vars = self$options$get('measures'),
