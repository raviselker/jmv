    ...
    private = list(
        .init = function() {
            ...
            for (i in seq_along(vars)) {
                ...
                if (!canBeNumeric(self$data[[var]]) && self$options$pearson) mtord <- TRUE
        ...
        .run = function() {
            ...
            if (hyp == 'pos') hyp <- 'greater' else if (hyp == 'neg') hyp <- 'less' else
                hyp <- 'two.sided'
            ...
            if (nVars > 1) {
                for (i in 2:nVars) {
                    ...
                    for (j in seq_len(i - 1)) {
                        ...
                        if (is.factor(rowVar) || is.factor(colVar)) mtord <- TRUE else
                            mtord <- FALSE
                        ...
                        if (mtord)
                            matrix$addFootnote(
                                rowNo = i,
                                paste0(colVarName, '[r]'),
                                .('Pearson correlation cannot be calculated for non-numeric values')
                            )

                        if (flag) {
                            if (!self$options$pearson || mtord) {
                            } else if (result$rp < .001) # do nothing
                                matrix$addSymbol(
                                    rowNo = i,
                                    paste0(colVarName, '[r]'),
                                    '***'
                            ...
                            } else if (result$rhop < .001) # do nothing
                                matrix$addSymbol(
                                    rowNo = i,
        ...
        .plot = function(image, ggtheme, ...) {
            ...
            if (length(columns) == 0) return(FALSE)
