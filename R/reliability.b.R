...
reliabilityClass <- R6::R6Class(
    ...
    active = list(
        ...
        alpha = function() {
            if (is.null(private$.alpha)) private$.alpha <- private$.computeAlpha()

            return(private$.alpha)
        },
        omega = function() {
            if (is.null(private$.omega)) private$.omega <- private$.computeOmega()

            return(private$.omega)
        },
        omegaDrop = function() {
            if (is.null(private$.omegaDrop)) private$.omegaDrop <- private$.computeOmegaDrop()

            return(private$.omegaDrop)
        },
        loadings = function() {
            if (is.null(private$.loadings)) private$.loadings <- private$.computeLoadings()
    ...
    private = list(
        ...
        .cleanData = function() {
            ...
            data <- list()
            for (item in items) data[[item]] <- jmvcore::toNumeric(self$data[[item]])
        ...
        .errorCheck = function(data) {
            ...
            if (any(allNAItems))
                jmvcore::reject(
                    ...
                    code = '',
                    item = items[allNAItems]
                )
            if (any(noVarItems))
                jmvcore::reject(
                    .("Item '{item}' has no variance"),
                    code = '',
                    item = items[noVarItems]
                )
        },
