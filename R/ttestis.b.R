...
ttestISClass <- R6::R6Class(
    ...
    private = list(
        .run = function() {
            ...
            if (is.null(groupVarName) || length(depVarNames) == 0) return()

            data <- select(self$data, varNames)

            for (name in depVarNames) data[[name]] <- jmvcore::toNumeric(data[[name]])
            ...
            if (any(depVarNames == groupVarName))
                jmvcore::reject(
                    .("Grouping variable '{a}' must not also be a dependent variable"),
                    code = "a_is_dependent_variable",
                    a = groupVarName
                )
            ...
            if (length(groupLevels) != 2)
                jmvcore::reject(
                    .("Grouping variable '{a}' must have exactly 2 levels"),
            ...
            if (self$options$miss == "listwise") {
                ...
                if (dim(data)[1] == 0)
                    jmvcore::reject(
                        .(
                            "Grouping variable '{a}' has less than 2 levels after missing values are excluded"
                        ),
                        code = "grouping_var_must_have_2_levels",
                        a = groupVarName
                    )
            ...
            for (depName in depVarNames) {
                ...
                if (self$options$miss == "perAnalysis") dataTTest <- naOmit(dataTTest)
                ...
                if (isError(levene)) {
                    ...
                    eqvTable$addFootnote(
                        rowKey = depName,
                        "f",
                        .("F-statistic could not be calculated")
                    )
                ...
                if (self$options$students) {
                    if (is.factor(dataTTest$dep))
                        res <- createError(.('Variable is not numeric')) else if (
                        any(is.infinite(dataTTest$dep))
                    )
                        res <- createError(.('Variable contains infinite values')) else
                        res <- try(
                            t.test(
                                dep ~ group,
                                data = dataTTest,
                    ...
                    if (isError(res)) {
                        ...
                        if (message == 'grouping factor must have exactly 2 levels')
                            message <- .(
                                'One or both groups do not contain enough observations'
                    ...
                    if (
                        !isError(levene) && !is.na(levene[1, "Pr(>F)"]) && levene[1, "Pr(>F)"] < .05
                    )
                ...
                if (self$options$welchs) {
                    ...
                    if (!isError(res)) {
                        ...
                        if (message == 'grouping factor must have exactly 2 levels')
                            ...
                            message <- .('Variable contains infinite values') else if (
                ...
                if (self$options$norm) {
                    ...
                    if (!is.null(footnote)) normTable$addFootnote(rowKey = depName, 'w', footnote)
                }
                ...
                if (self$options$bf) {
                    ...
                    if (isError(res)) {
                        ...
                        if (message == 'grouping factor must have exactly 2 levels')
                            ...
                            message <- .('Variable is not numeric') else if (
                            message == 'data are essentially constant'
                        )
                            message <- .('All observations are tied')
                        ...
                        bf <- extracted$bf[1]
                        if (is.na(error)) error <- NaN
                        if (!is.numeric(bf)) bf <- NaN
                ...
                if (self$options$plots) {
                    ...
                    if (nrow(dataTTest) > 0) {
                        ...
                        means <- aggregate(
                            dataTTest$dep,
                            by = list(dataTTest$group),
                            function(x) tryNaN(mean(x)),
                            simplify = FALSE
                        )
                        cies <- aggregate(
                            dataTTest$dep,
                            by = list(dataTTest$group),
                            function(x) {
                                tail * tryNaN(sd(x)) / sqrt(length(x))
                            },
                            simplify = FALSE
                        )
                        medians <- aggregate(
                        ...
                        if (all(is.na(plotData$stat))) image$setState(NULL) else
        ...
        .init = function() {
            ...
            groups <- NULL
            if (!is.null(groupName)) groups <- base::levels(self$data[[groupName]])
            if (length(groups) != 2) groups <- c('Group 1', 'Group 2')
        ...
        .desc = function(image, ggtheme, theme, ...) {
            ...
            plot <- ggplot(data = image$state, aes(x = group, y = stat, shape = type)) +
                geom_errorbar(
                    aes(x = group, ymin = stat - cie, ymax = stat + cie, width = .1),
                    size = .8,
                    colour = theme$color[2],
                    position = pd
                ) +
                geom_point(
                    aes(x = group, y = stat, shape = type),
                    color = theme$color[1],
                    fill = theme$fill[1],
                    size = 3,
                    position = pd
        ...
        .sourcifyOption = function(option) {
            if (option$name %in% c('deps', 'group')) return('')
