...
contTablesPairedClass <- R6::R6Class(
    ...
    private = list(
        .cleanData = function() {
            ...
            if (!is.null(rowVarName)) data[[rowVarName]] <- as.factor(data[[rowVarName]])
            if (!is.null(colVarName)) data[[colVarName]] <- as.factor(data[[colVarName]])
            if (!is.null(countsName)) data[[countsName]] <- jmvcore::toNumeric(data[[countsName]])
        ...
        .run = function() {
            ...
            if (!is.null(countsName)) {
                ...
                if (any(is.infinite(countCol))) jmvcore::reject(.('Counts may not be infinite'))
            }
            ...
            if (!is.null(countsName)) result <- stats::xtabs(countCol ~ rowVar + colVar) else
            ...
            if (base::inherits(exact, 'try-error') || is.na(exact$estimate)) {
                ...
                if (self$options$exact && !requireNamespace('exact2x2', quietly = TRUE))
                    stop(
                        'exact2x2 must be installed to calculate an exact log odds ratio',
                        call. = FALSE
                    )
            } else {
            ...
            if (base::inherits(wocor, 'try-error')) {
                ...
                if (error == "'x' must be square with at least two rows and columns")
                    error <- .('McNemar requires a 2x2 table') else if (
        ...
        .init = function() {
            ...
            hasSubRows <- sum(self$options$pcRow, self$options$pcCol) > 0
            ...
            for (j in seq_along(subNames)) {
                subName <- subNames[[j]]
                if (j == 1) v <- '(pcRow || pcCol)' else v <- visible[j]
            ...
            for (i in seq_len(nrow(rows))) {
                ...
                if (i == 1) freqs$addFormat(rowNo = i, 1, Cell.BEGIN_GROUP) else if (
                    i == nrow(rows) - 1
                )
                    freqs$addFormat(rowNo = i, 1, Cell.END_GROUP) else if (i == nrow(rows))
        ...
        .formula = function() {
            ...
            jmvcore:::composeFormula(
                self$options$counts,
                list(list(self$options$rows, self$options$cols))
            )
        }
    )
)
