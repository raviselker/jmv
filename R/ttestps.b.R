...
ttestPSClass <- R6::R6Class(
    ...
    private = list(
        .run = function() {
            ...
            if (self$options$get('miss') == 'listwise') data <- naOmit(data)

            if (self$options$get("hypothesis") == "oneGreater") Ha <- "greater" else if (
                self$options$get("hypothesis") == "twoGreater"
            )
            ...
            for (i in seq_along(pairs)) {
                ...
                if (is.null(name1) || is.null(name2)) next()
                ...
                if (!isError(stud)) {
                    ...
                    if (message == "not enough 'x' observations")
                        message <- .(
                            'One or both variables do not contain enough observations'
                ...
                if (!isError(wilc)) {
                    ...
                    if (message == "not enough 'x' observations")
                        ...
                        message <- .('One or both variables contain infinite values') else if (
                        message ==
                            'cannot compute confidence interval when all observations are tied'
                    )
                        message <- .('All observations are tied') else if (
                ...
                if (self$options$plots) {
                    ...
                    cies <- c(
                        tail * tryNaN(sd(column1)) / sqrt(length(column1)),
                        tail * tryNaN(sd(column2)) / sqrt(length(column2))
                    )
        ...
        .desc = function(image, ggtheme, theme, ...) {
            ...
            plot <- ggplot(data = image$state, aes(x = group, y = stat, shape = type)) +
                geom_errorbar(
                    aes(x = group, ymin = stat - cie, ymax = stat + cie, shape = type, width = .2),
                    size = .8,
                    colour = theme$color[1],
                    position = pd
                ) +
                geom_point(
                    aes(x = group, y = stat, colour = type, shape = type),
                    fill = theme$fill[1],
                    size = 3,
                    colour = theme$color[1],
                    position = pd
