    ...
    private = list(
        ...
        .run = function() {
            ...
            if (length(self$options$vars) > 0) {
                ...
                if (self$options$postPlots) private$.preparePostPlots(results)
            }
        },
        ...
        .initTable = function() {
            ...
            hyp <- self$options$hypothesis
            if (hyp == 'greater') op <- '>' else if (hyp == 'less') op <- '<' else op <- '\u2260'
            ...
            ciWidthBayesTitle <- jmvcore::format(
                ciWidthTitleString,
                ciWidth = self$options$ciBayesWidth
            )
            ...
            for (var in self$options$vars) {
                ...
                table$addFormat(
                    rowKey = paste0(var, '`', levels[1]),
                    'var',
                    jmvcore::Cell.BEGIN_GROUP
                )
                table$addFormat(
                    rowKey = paste0(var, '`', levels[length(levels)]),
                    'var',
                    jmvcore::Cell.END_GROUP
                )
            }
        },
        .initPostPlots = function() {
            ...
            for (var in vars) {
                ...
                if (self$options$areCounts) {
                    ...
                    if (!is.factor(column)) levels <- levels(as.factor(column)) else
                        levels <- levels(column)
                }

                for (level in levels) group$addItem(level)
            }
        },
        ...
        .populateTable = function(results) {
            ...
            for (var in self$options$vars) {
                ...
                for (i in seq_along(levels)) levels[i] <- varResults[[i]]$level
            ...
            for (var in vars) {
                ...
                for (i in seq_along(levels)) {
                    ...
                    if (hyp == "notequal") {
                        ...
                        prior <- ifelse(
                            x >= prop,
                            dbeta(x, a, b) / pbeta(prop, a, b, lower.tail = FALSE),
                            0
                        )
                        post <- ifelse(
                            x >= prop,
                            dbeta(x, a + k, b + n - k) /
                                pbeta(prop, a + k, b + n - k, lower.tail = FALSE),
                            0
                        )
                        like <- ifelse(
                            x >= prop,
                            dbeta(x, 1 + k, 1 + n - k) /
                                pbeta(prop, 1 + k, 1 + n - k, lower.tail = FALSE),
                            0
                        )
                    } else {
                        prior <- ifelse(
                            x <= prop,
                            dbeta(x, a, b) / pbeta(prop, a, b, lower.tail = TRUE),
                            0
                        )
                        post <- ifelse(
                            x <= prop,
                            dbeta(x, a + k, b + n - k) /
                                pbeta(prop, a + k, b + n - k, lower.tail = TRUE),
                            0
                        )
                        like <- ifelse(
                            x <= prop,
                            dbeta(x, 1 + k, 1 + n - k) /
                                pbeta(prop, 1 + k, 1 + n - k, lower.tail = TRUE),
                            0
                        )
                    }
                    ...
                    y <- c(prior, like, post)
                    g <- factor(
                        rep(c(.("Prior"), .("Likelihood"), .("Posterior")), each = length(prior)),
        ...
        .counts = function(var) {
            ...
            if (self$options$areCounts) {
                ...
                if (jmvcore::canBeNumeric(varData)) counts <- jmvcore::toNumeric(varData) else
                    counts <- suppressWarnings(as.numeric(as.character(varData)))
            } else {
                levels <- base::levels(varData)
                if (length(levels) == 0) levels <- paste(sort(unique(varData)))
        ...
        .bf = function(k, n, prop, hyp = 'two.sided', ciWidth = 0.95, a = 1, b = 1) {
            ...
            if (hyp == 'two.sided') {
                ...
                heightPosterior <- dbeta(prop, a + k, b + n - k) /
                    pbeta(prop, a + k, b + n - k, lower.tail = FALSE)
