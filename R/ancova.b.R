...
ancovaClass <- R6::R6Class(
    ...
    private = list(
        ...
        .init = function() {
            ...
            if (length(factors) == 0 || length(modelTerms) == 0) return()
        ...
        .run = function() {
            ...
            if (is.null(dep) || length(factors) == 0 || length(modelTerms) == 0) return()
        ...
        .compute = function(data) {
            ...
            perfectFitErrorMessage <- .(
                "Residual sum of squares and/or degrees of freedom are zero, indicating a perfect fit"
            )
            ...
            suppressWarnings({
                ...
                for (contrast in self$options$contrasts) {
                    ...
                    stats::contrasts(data[[contrast$var]]) <- private$.createContrasts(
                        levels,
                        contrast$type
                    )
                }
                ...
                if (self$options$ss == '1') {
                    ...
                    results <- try(
                        car::Anova(private$.model, type = 2, singular.ok = FALSE),
                        silent = TRUE
                    )
                    if (isError(results)) {
                        ...
                        results <- try(
                            car::Anova(private$.model, type = 2, singular.ok = TRUE),
                            silent = TRUE
                        )
                    }
                } else {
        ...
        .initMainTable = function() {
            ...
            if (length(modelTerms) > 0) {
                for (i in seq_along(modelTerms)) {
                    ...
                    if (i == 1)
                        table$addFormat(rowKey = term, col = 1, format = Cell.BEGIN_GROUP) else if (
                        i == length(modelTerms)
                    )
        ...
        .initContrastTables = function() {
            ...
            for (contrast in self$options$contrasts) {
                ...
                for (label in labels) table$addRow(rowKey = label, list(contrast = label))
            }
        },
        .initPostHoc = function() {
            ...
            bsLevels <- list()
            for (i in seq_along(bs)) bsLevels[[bs[i]]] <- levels(data[[bs[i]]])
            ...
            for (ph in phTerms) {
                ...
                ciTitleES <- jmvcore::format(
                    .('{ciWidth}% Confidence Interval'),
                    ciWidth = self$options$postHocEsCiWidth
                )
                table$addColumn(
                    name = 'd',
                    title = .("Cohen's d"),
                ...
                table$addColumn(
                    ...
                    title = .('Lower'),
                    type = 'number',
                    visible = "(postHocES:d && postHocEsCi)",
                    superTitle = ciTitleES
                )
                table$addColumn(
                    name = 'dupper',
                    title = .('Upper'),
        ...
        .populateMainTable = function(results) {
            ...
            if (!is.null(singular)) table$setNote('singular', singular)
            ...
            for (rowKey in table$rowKeys) {
                if (identical(rowKey, '')) index <- rowCount else if (identical(rowKey, '.'))
                    index <- 0 else index <- matchSet(rowKey, decomposed)
        ...
        .populatePostHoc = function(data) {
            ...
            for (ph in terms) {
                ...
                resultRows <- lapply(
                    strsplit(as.character(none$contrast), ' - '),
                ...
                for (i in seq_along(tableRows)) {
                    ...
                    if (length(index) == 1) {
                        ...
                        row <- list()
                        row[['md']] <- if (reverse) -none[index, 'estimate'] else
                            none[index, 'estimate']
                        row[['se']] <- none[index, 'SE']
                        row[['df']] <- none[index, 'df']
                        row[['t']] <- if (reverse) -none[index, 't.ratio'] else
                            none[index, 't.ratio']
        ...
        .populateNormality = function() {
            if (!self$options$norm) return()

            residuals <- self$residuals
            if (is.null(residuals)) return()
        ...
        .qqPlot = function(image, ggtheme, theme, ...) {
            ...
            if (is.null(residuals) || all(is.na(residuals))) return()
        ...
        .contrastLabels = function(levels, type) {
            ...
            if (length(levels) <= 1) {
                ...
                for (i in seq_len(nLevels - 1)) labels[[i]] <- paste(levels[i + 1], '-', levels[1])
            } else if (type == 'deviation') {
                all <- paste(levels, collapse = ', ')
                for (i in seq_len(nLevels - 1)) labels[[i]] <- paste(levels[i + 1], '-', all)
                ...
                for (i in seq_len(nLevels - 1)) labels[[i]] <- paste(levels[i], '-', levels[i + 1])
        ...
        .createContrasts = function(levels, type) {
            ...
            if (type == 'simple') {
                ...
                for (i in 1:ncol(contrast)) contrast[, i] <- contrast[, i] / (i + 1)
        ...
        .modelTerms = function() {
            ...
            if (length(modelTerms) == 0) modelTerms <- private$.ff()
        ...
        .ff = function() {
            ...
            covs <- NULL
            if ('covs' %in% names(self$options)) covs <- self$options$covs

            for (covariate in covs) modelTerms[[length(modelTerms) + 1]] <- covariate
        ...
        .sourcifyOption = function(option) {
            ...
            if (option$name %in% c('factors', 'dep', 'covs', 'modelTerms')) return('')

            if (name == 'contrasts') {
                i <- 1
                while (i <= length(value)) {
                    item <- value[[i]]
                    if (item$type == 'none') value[[i]] <- NULL else i <- i + 1
                }
                if (length(value) == 0) return('')
            } else if (name == 'modelTerms') {
                if (base::identical(as.list(value), private$.ff())) return('')
        ...
        .emmPlotSize = function(emm) {
            ...
            levels <- list()
            for (i in seq_along(emm)) levels[[emm[i]]] <- levels(data[[emm[i]]])
    ...
    active = list(
        ...
        finalData = function() {
            if (self$ready && is.null(private$.finalData)) {
                ...
                if (!is.null(dep)) data[[dep]] <- jmvcore::toNumeric(data[[dep]])

                for (factor in factors) data[[factor]] <- as.factor(data[[factor]])

                for (covariate in covs) data[[covariate]] <- jmvcore::toNumeric(data[[covariate]])
