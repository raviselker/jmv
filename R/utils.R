...
aggregate <- function(x, by, FUN, ..., simplify = TRUE, drop = TRUE) {
    ...
    if (is.null(names(by)) && length(by))
        names(by) <- paste("Group", seq_along(by), sep = ".") else {
        nam <- names(by)
        ind <- which(!nzchar(nam))
        names(by)[ind] <- paste("Group", ind, sep = ".")
    }
    nrx <- NROW(x)
    if (any(lengths(by) != nrx)) stop("arguments must have same length")
    ...
    z <- lapply(x, function(e) {
        ...
        if (simplify && length(len <- unique(lengths(ans))) == 1L) {
            if (len == 1L) {
                ...
                if (!is.null(cl1) && all(sapply(cl, function(x) identical(x, cl1))))
                    class(ans) <- cl1
            } else if (len > 1L)
                ans <- matrix(
                    ...
                    byrow = TRUE,
                    dimnames = {
                        if (!is.null(nms <- names(ans[[1L]]))) list(NULL, nms) else NULL
                    }
                )
        }
        ans
...
tapply = function(X, INDEX, FUN = NULL, ..., simplify = TRUE, drop = TRUE) {
    ...
    if (!all(lengths(INDEX) == length(X))) stop("arguments must have same length")
    namelist <- lapply(INDEX, levels)
    extent <- lengths(namelist, use.names = FALSE)
    cumextent <- cumprod(extent)
    if (cumextent[nI] > .Machine$integer.max) stop("total number of levels >= 2^31")
