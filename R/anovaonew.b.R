#' @importFrom jmvcore .
anovaOneWClass <- if (requireNamespace('jmvcore'))
    R6::R6Class(
        "anovaOneWClass",
        ...
        private = list(
            ...
            .run = function() {
                ready <- TRUE
                if (is.null(self$options$group) || length(self$options$deps) < 1) return()
            ...
            .compute = function(data) {
                ...
                for (dep in self$options$deps) {
                    ...
                    if (self$options$miss != 'listwise') dataA <- na.omit(dataA)
            ...
            .initAnovaTable = function() {
                ...
                if (self$options$fishers && !self$options$welchs)
                    table$setTitle(.("One-Way ANOVA (Fisher's)")) else if (
                    self$options$welchs && !self$options$fishers
                )
            ...
            .initDescTable = function() {
                ...
                for (dep in self$options$deps) {
                    for (i in seq_along(levels)) {
                        ...
                        if (i == 1)
                            table$addFormat(
                                rowKey = paste0(dep, levels[i]),
                                col = 1,
                                jmvcore::Cell.BEGIN_GROUP
                            )
                    }
                }
            },
            ...
            .populateShapiroWilkTable = function(results) {
                ...
                for (dep in self$options$deps) {
                    ...
                    table$setRow(rowKey = dep, row)
                    if (!is.null(footnote)) table$addFootnote(rowKey = dep, 'w', footnote)
                }
            },
                    ...
                    for (j in 1:(length(levels) - 1)) {
                        for (k in (j + 1):length(levels)) {
                            ...
                            if (self$options$phFlag) {
                                p <- r[index, 'p']
                                if (is.na(p)) {
                                } else if (p < .001) # do nothing
                                    table$addSymbol(rowNo = j, paste0(g2, '[md]'), '***') else if (
                                    p < .01
                                )
                                    table$addSymbol(rowNo = j, paste0(g2, '[md]'), '**') else if (
                                    p < .05
            ...
            .desc = function(image, ggtheme, theme, ...) {
                ...
                p <- ggplot2::ggplot(data = image$state$df, ggplot2::aes(x = levels, y = mean)) +
                    ggplot2::geom_errorbar(
                        ggplot2::aes(ymin = mean - ci, ymax = mean + ci, width = .1),
                        size = .8,
                        color = theme$color[2]
                    ) +
                    ggplot2::geom_point(
                        ggplot2::aes(color = errorType),
                        fill = theme$fill[1],
                        size = 3,
                        shape = 21
                    ) +
            ...
            .prepareQQPlot = function(results) {
                ...
                for (dep in self$options$deps) {
                    ...
                    if (nrow(df) > 10000) df <- df[!duplicated(round(df$x, 2)), ]
            ...
            .cleanData = function() {
                ...
                for (dep in self$options$deps) data[[dep]] <- jmvcore::toNumeric(data[[dep]])

                if (self$options$miss == 'listwise') data <- na.omit(data)
            ...
            .descPlotSize = function() {
                ...
                if (is.null(group)) return(c(300, 350))
            ...
            .postHoc = function(desc, method = 'gamesHowell') {
                ...
                stats <- lapply(1:ncol(combs), function(x) {
                    ...
                    if (method == 'gamesHowell') {
                        t <- md /
                            sqrt(
                                (vars[combs[1, x]] / ns[combs[1, x]]) +
                                    (vars[combs[2, x]] / ns[combs[2, x]])
                            )

                        df <- (vars[combs[1, x]] /
                            ns[combs[1, x]] +
                            vars[combs[2, x]] / ns[combs[2, x]])^2 /
                            ((vars[combs[1, x]] / ns[combs[1, x]])^2 /
                                (ns[combs[1, x]] - 1) +
