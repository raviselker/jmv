...
testthat::test_that('All basic options in the ancova work (sunny)', {
    ...
    testthat::expect_equal(
        c(527.821, 62.06, 39.788, 108.723, 317.25, 414.61),
        mainTable[['ss']],
        tolerance = 1e-3
    )
    testthat::expect_equal(c(6, 2, 1, 1, 2, 11), mainTable[['df']])
    testthat::expect_equal(
        c(87.97, 31.03, 39.788, 108.723, 158.625, 37.692),
        mainTable[['ms']],
        tolerance = 1e-3
    )
    testthat::expect_equal(
        c(1.534, 0.823, 1.056, 2.885, 4.208, NA),
        mainTable[['F']],
        tolerance = 1e-3
    )
    testthat::expect_equal(
        c(0.255, 0.464, 0.326, 0.118, 0.044, NA),
        mainTable[['p']],
        tolerance = 1e-3
    )
    testthat::expect_equal(
        c(NA, 0.066, 0.042, 0.115, 0.337, NA),
        mainTable[['etaSq']],
        tolerance = 1e-3
    )
    testthat::expect_equal(
        c(NA, 0.13, 0.088, 0.208, 0.433, NA),
        mainTable[['etaSqP']],
        tolerance = 1e-3
    )
    testthat::expect_equal(
        c(NA, -0.014, 0.002, 0.072, 0.247, NA),
        mainTable[['omegaSq']],
        tolerance = 1e-3
    )
})
